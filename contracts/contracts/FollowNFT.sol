 ...
 */
contract FollowNFT is RejigNFTBase, IFollowNFT {
    struct Snapshot {
        uint128 blockNumber;
        uint128 value;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return IRejig(HUB).getFollowNFTURI(_profileId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        address followModule = IRejig(HUB).getFollowModule(_profileId);

        _moveDelegate(fromDelegatee, toDelegatee, 1);

        super._beforeTokenTransfer(from, to, tokenId);
        IRejig(HUB).emitFollowNFTTransferEvent(_profileId, tokenId, from, to);
