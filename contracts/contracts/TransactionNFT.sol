 ...
 *
 * @notice This contract is the NFT that is minted upon transactioning a given profile. It is cloned upon first transaction for a
 * given profile, and includes built-in governance power and delegation mechanisms.
 *
 * NOTE: This contract assumes total NFT supply for this transaction NFT will never exceed 2^128 - 1
 */
contract TransactionNFT is RejigNFTBase, ITransactionNFT {
    struct Snapshot {
        uint128 blockNumber;
        uint128 value;
    }
    ...
    // We create the TransactionNFT with the pre-computed HUB address before deploying the hub.
    ...
    /// @inheritdoc ITransactionNFT
    function mint(address to, uint tokenId) external override {
    ...
    /// @inheritdoc ITransactionNFT
    function transferFrom(address from, address to, uint nftId) public override(ERC721Time, ITransactionNFT){
    ...
    function name() public view override returns (string memory) {
        string memory handle = IRejig(HUB).getHandle(_profileId);
        return string(abi.encodePacked(handle, Constants.TRANSACTION_NFT_NAME_SUFFIX));
    }

    function symbol() public view override returns (string memory) {
        string memory handle = IRejig(HUB).getHandle(_profileId);
        bytes4 firstBytes = bytes4(bytes(handle));
        return string(abi.encodePacked(firstBytes, Constants.TRANSACTION_NFT_SYMBOL_SUFFIX));
    }
    ...
    /**
     * @dev This returns the transaction NFT URI fetched from the hub.
     */
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        if (!_exists(tokenId)) revert Errors.TokenDoesNotExist();
        return IRejig(HUB).getTransactionNFTURI(_profileId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        address transactionModule = IRejig(HUB).getTransactionModule(_profileId);

        _moveDelegate(fromDelegatee, toDelegatee, 1);

        super._beforeTokenTransfer(from, to, tokenId);
        IRejig(HUB).emitTransactionNFTTransferEvent(_profileId, tokenId, from, to);
        if (transactionModule != address(0)) {
            ITransactionModule(transactionModule).transactionModuleTransferHook(_profileId, from, to, tokenId);
        }
    }
