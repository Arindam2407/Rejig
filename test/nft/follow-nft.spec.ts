...
makeSuiteCleanRoom('Follow NFT', function () {
  ...
  context('meta-tx', function () {
    ...
    beforeEach(async function () {
      await expect(rejig.connect(testWallet).follow([FIRST_PROFILE_ID], [[]])).to.not.be.reverted;
      followNFT = FollowNFT__factory.connect(await rejig.getFollowNFT(FIRST_PROFILE_ID), user);
    });
  });
});
